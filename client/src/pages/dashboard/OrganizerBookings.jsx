...
import { Search, Calendar, TicketCheck, Eye } from 'lucide-react';
  ...
  return (
    <div>
      <div style={{ marginBottom: 'var(--space-5)' }}>
        ...
        <p style={{ color: 'var(--text-muted)' }}>View and manage bookings for your temples</p>
      </div>
      ...
      {loading ? <Loader text="Loading bookings..." /> : bookings.length === 0 ? (
        <EmptyState icon="🎫" title="No bookings found" message="Bookings for your temples will appear here." />
      ) : (
        <>
          <div style={{
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ background: 'var(--bg-secondary)' }}>
                    {['Booking ID', 'Temple', 'Devotee', 'Date', 'Devotees', 'Amount', 'Status'].map((h) => (
                      <th key={h} style={{
                    ...
                    <tr key={b._id} style={{ borderBottom: '1px solid var(--border-light)' }}>
                      ...
                      <td style={{ padding: 'var(--space-3) var(--space-4)', fontSize: 'var(--fs-sm)', color: 'var(--text-muted)' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                          <Calendar size={12} /> {formatDate(b.date)}
                        </div>
                      </td>
